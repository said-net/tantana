...
import { BiLock, BiMoney, BiPhone, BiUser } from "react-icons/bi";
...
function FromOperator({ open, setOpen, partners }) {
    const [state, setState] = useState({ name: '', phone: '+998', location: '', about: '', mortgage: '', price: '', services: '', date: '', partner: '' });
    const [disabled, setDisabled] = useState(false);
    const dp = useDispatch();
    function Submit() {
        setDisabled(true);
        axios.post(`${API}/partner/create-from-operator`, state, {
            headers: {
            ...
            if (!ok) {
                ...
                toast.success(msg);
                setState({ name: '', phone: '+998', location: '', about: '', mortgage: '', price: '', services: '', date: '', partner: '' });
                dp(setRefreshOrder());
                setOpen(false);
            }
        }).catch(() => {
                ...
                <DialogBody className="w-full border-y h-[500px] overflow-y-scroll">
                    <div className="flex items-center justify-start flex-col w-full">
                        <div className="flex items-center justify-center w-full mb-[10px]">
                            <Select onChange={e => setState({ ...state, partner: e })} label={"Hamkor tanlang"}>
                                {
                                    partners.map((p, key) => {
                                        return (
